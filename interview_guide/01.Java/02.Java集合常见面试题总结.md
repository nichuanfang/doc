...
![Java 集合框架概览](./img/java-collection-hierarchy.png)
...
![unlink 方法逻辑](./img/linkedlist-unlink.jpg)
...
![img](./img/redisimage-20220412110853807.png)
...
![双向链表](./img/bidirectional-linkedlist.png)

**双向循环链表：** 最后一个节点的 next 指向 head，而 head 的 prev 指向最后一个节点，构成一个环。

![双向循环链表](./img/bidirectional-circular-linkedlist.png)
...
`ArrayList`是`List`接口的实现类，它是支持根据需要而动态增长的数组。java中标准数组是定长的，在数组被创建之后，它们不能被加长或缩短。这就意味着在创建数组时需要知道数组的所需长度，但有时我们需要动态程序中获取数组长度。ArrayList就是为此而生的,但是它不是线程安全的，外ArrayList按照插入的顺序来存放数据
...
详见笔主的这篇文章: [ArrayList 扩容机制分析](https://juejin.cn/post/7092645723623129118)。
...
![img](./img/fail-fast-and-fail-safe-insert-100-values.png)
...
![img](./img/fail-fast-and-fail-safe-copyonwritearraylist.png)
...
![BlockingQueue 的实现类](./img/blocking-queue-hierarchy.png)
...
![TreeMap 继承关系图](./img/treemap_hierarchy.png)
...
![jdk1.8 之前的内部结构-HashMap](./img/jdk1.7_hashmap.png)
...
![jdk1.8之后的内部结构-HashMap](./img/jdk1.8_hashmap.png)
...
![Hashtable 的内部结构](./img/jdk1.7_hashmap.png)

https\://www\.cnblogs.com/chengxiao/p/6842045.html>

**JDK1.7 的 ConcurrentHashMap**：

![Java7 ConcurrentHashMap 存储结构](./img/java7_concurrenthashmap.png)
...
![Java8 ConcurrentHashMap 存储结构](./img/java8_concurrenthashmap.png)
