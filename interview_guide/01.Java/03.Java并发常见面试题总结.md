...
![进程示例图片-Windows](./img/进程示例图片-Windows.png)
...
![常见的三种线程模型](./img/three-types-of-thread-models.png)
...
![RUNNABLE-VS-RUNNING](./img/RUNNABLE-VS-RUNNING.png)
...
![jconsole](./img/jdk-home-bin-jconsole.png)
...
![jconsole 检测死锁](./img/jconsole-check-deadlock.png)

![jconsole 检测到死锁](./img/jconsole-check-deadlock-done.png)
...
![JMM(Java 内存模型)](./img/jmm.png)

![JMM(Java 内存模型)强制在主存中进行读取](./img/jmm2.png)
...
![JUC原子类概览](./img/JUC原子类概览-20230814005211968.png)
...
![synchronized关键字原理](./img/synchronized-principle.png)
...
![执行 monitorenter 获取锁](./img/synchronized-get-lock-code-block.png)

对象锁的拥有者线程才可以执行 `monitorexit` 指令来释放锁。在执行 `monitorexit` 指令后，将锁计数器设为 0，表明锁被释放，其他线程可以尝试获取锁。

![执行 monitorexit 释放锁](./img/synchronized-release-lock-block.png)
...
![](./img/reentrantlock-class-diagram.png)
...
![](./img/reentrantreadwritelock-class-diagram.png)
...
![ThreadLocal 数据结构](./img/threadlocal-data-structure.png)

`ThreadLocalMap`是`ThreadLocal`的静态内部类。

![ThreadLocal内部类](./img/thread-local-inner-class.png)
...
![](./img/threadpoolexecutor-construtors.png)
...
![](./img/executors-new-thread-pool-methods.png)
...
![线程池各个参数的关系](./img/relationship-between-thread-pool-parameters.png)
...
![调整阻塞队列大小和最大线程数](./img/threadpool-reject-2-threadpool-reject-01.png)
...
![将一部分任务保存到MySQL中](./img/threadpool-reject-2-threadpool-reject-02.png)
...
![图解线程池实现原理](./img/thread-pool-principle.png)
...
![](./img/threadpoolexecutor-methods.png)
...
![动态配置线程池参数最终效果](./img/meituan-dynamically-configuring-thread-pool-parameters.png)
...
![《后端面试高频系统设计&场景题》](./img/back-end-interview-high-frequency-system-design-and-scenario-questions-fengmian.png)
...
![ThreadPoolExecutor构造函数](./img/common-parameters-of-threadpool-workqueue.jpg)
...
![](./img/completablefuture-class-diagram.jpg)
...
![](./img/image-20210902093026059.png)
...
![CLH 锁的队列结构](./img/clh-lock-queue-structure.png)
...
![CLH 变体队列结构](./img/clh-queue-structure-bianti.png)

AQS(`AbstractQueuedSynchronizer`)的核心原理图：

![CLH 变体队列](./img/clh-queue-state.png)
