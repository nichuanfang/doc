...
### ⭐ == 与 equals() 的区别；为什么重写 equals() 要重写 hashCode()**
- `==`：基本类型比较值，引用类型比较内存地址。
...
### 异常体系：Exception 与 Error 的区别，Checked 与 Unchecked 异常**
...
### ⭐ try-catch-finally 执行顺序，finally 中 return 的坑**
...
### Java 8 新特性：Lambda 表达式、Stream API、函数式接口、Optional**
...
### ⭐ ArrayList 与 LinkedList 的区别及底层实现**
...
### ⭐ HashMap 与 Hashtable、ConcurrentHashMap 的区别**
...
### ⭐ Iterator 遍历原理，fail-fast 与 fail-safe 机制**
...
### ⭐ synchronized 关键字原理（对象头、Monitor）**
...
### ⭐ volatile 关键字的作用：可见性、有序性，为什么不保证原子性**
...
### ⭐ CountDownLatch、CyclicBarrier、Semaphore 的区别与使用场景**
...
### ⭐ ConcurrentHashMap 的实现原理（JDK 7 分段锁 vs JDK 8 CAS+synchronized）**
...
### ⭐ 索引的数据结构：为什么用 B+ 树而不是 B 树/红黑树/哈希表**
...
### ⭐ redo log、undo log、binlog 的作用及区别**
...
### ⭐ 两阶段提交（2PC）如何保证 redo log 与 binlog 一致性**
...
### ⭐ 如何保证缓存与数据库的一致性（Cache Aside 模式等）**
...
### ⭐ Redis 分布式锁的实现（SET NX EX、Redlock）**
...
### ⭐ Redis Cluster 集群分片原理（哈希槽 16384）**
...
### Redis 与本地缓存（如 Caffeine）的多级缓存设计**
...
### Bean 的作用域（singleton、prototype 等）及线程安全问题**
...
### ⭐ 事务传播机制（REQUIRED、REQUIRES_NEW 等）**
...
### Spring 中用到的设计模式（工厂、代理、单例、模板方法、观察者等）**
...
### ⭐ Spring Boot 自动装配（Auto Configuration）原理**
...
### Spring Boot 常用 starter 及自定义 starter 的实现思路**
...
### Spring Boot 配置文件加载顺序及多环境配置（profile）**
...
### Spring Bean 是否线程安全，如何处理有状态 Bean**
...
### Spring Boot 常见监控与健康检查（Actuator）**
...
### 动态 SQL 常用标签（if、choose、foreach、where、set 等）**
...
### ⭐ MyBatis 如何实现分页（物理分页 vs 逻辑分页，PageHelper 原理）**
...
### MyBatis-Plus 相比原生 MyBatis 的增强点**
...
### MyBatis 如何处理数据库与 Java 类型的映射（TypeHandler）**
...
### Netty 的核心组件及为什么用 Netty 而不是原生 NIO**
